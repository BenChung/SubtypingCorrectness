...
\begin{abstract}
  ...
  features did so by normalizing types to disjunctive normal form
...
\subseteq \denotes{t_2}$}. From this we derive an \emph{forall-exists}
...
\noindent While this rule system might seems to make intuitive sense, it does
not match the semantic intuition for subtyping. While it attempts to express
the same forall-exists relation that arises from the semantic intuition, it is
possible to construct judgments where forall-exists and this rule system
differ. For instance, consider the following judgment:
...
We have presented an implementation that uses lists to represent choice
strings. It thus requires allocation when adding elements to the list and
