...
determine which method to invoke at each call site; for this, it searches
the most specific applicable method (according to subtyping) that applies
for a given invocation. 
The following snippet shows three declarations of
multiplication:
...
To allow the expression of complex relationships in type signature, Julia
offers programmers a rich type language. Julia's types include nominal single
subtyping, union types, existential types, covariant tuples, invariant
parametric datatypes, and singletons. These are all widely used in
libraries, but pose challenges for subtyping.  Julia's design~\cite{BezansonEKS17} was 
inspired by semantic subtyping~\cite{Frisch02}, and while its
types do not precisely abide by a semantic set-theoretic intuition, its
subtyping
algorithm
must handle many of the same problems.
...
problem. However, doing so only reveals a deeper problem caused by the presence
