...
\begin{lemma}[Iterator number monotonicity]\label{inum_mono}
...
\c{exist} \c a \c b \c{ia} \c{ib} holds iff $\exists \c t \in \irch{ib}, \c a(\c{ia}) = \c t$
\end{theorem}
...
\c{allexist} \c a \c b \c{ia} holds iff $\forall \c{a'} \in \irch{ia}, \exists \c{b'} \in \denotes{\c b}, \c{a'} = \c{b'}$
\end{theorem}
...
\c{sub} \c a \c b holds iff $\forall \c{a'} \in \denotes{\c a}, \exists \c{b'} \in \denotes{\c b}, \c{a'} = \c{b'}$
\end{theorem}
...
\noindent
The translated input and output of the tree iterator \c{next}
function is of the form described in section~\ref{cs}. 

\begin{lemma}[Structure of linearized {\tt next\_tree}]
\label{lem:snt}
For some type \c{t} and tree iterators \c{it it'}, if \c{next} \c t \c {it} $=$ \c{it'}, there exists \c{c'}, \c{c''} made up of \Rs, and \c{c'''} consisting of \Ls such that \c{itp t it} $=$ \c{c'} \c @ $(\c{Left} :: \c{c''})$ and \c{itp t it'} $=$ \c{c'} \c @ $(\c{Right} :: \c{c'''})$.
\end{lemma}
\noindent
...
\begin{lemma}[Correctness of step\_ctx]\label{sctxcorr}
...
\begin{lemma}[Equal Initial States]\label{eqinit}
$\c{itp} (\c{initial}\;\c t) = \c{pad}\;\c t \;\c{[]}$
\end{lemma}

The functions \verb|exists| and \verb|allexists| for choice string based
