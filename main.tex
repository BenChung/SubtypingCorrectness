...
we can reason about all reachable states at once. We show that all iterators
...
\begin{lemma}[Monotonicity of \c{inum}]
...
from all configruations. If we can prove that our desired property holds
...
uses tree iterators instead of choice strings. The tree iterator \c{sub} works by
simultaneously deconstructing the type and the iterator over the type, following the
choices specified by the iterator at each choice point encountered. 
...
We consider the denotation of a tree iterator state $\irch{i}$ to be the set
of reachable union-free types from the current state. We also write $\c a(\c
{ia})$  to indicate the union-free type produced from the type \c a by
following the choices in the tree iterator \c{ia}.
...
\begin{lemma}[Tree iterator denotational equivalence]\label{triter_eq}
