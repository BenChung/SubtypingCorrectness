...
iterate over. The possible states at primitives and tuples are simple, as in
the former case there are no alternative choices to make and in the latter the
choices are those for the children of the tuple. For unions, the iterator state 
may go either left or right. In this manner, this structure explicitly assigns
choices to specific unions in the type tree thereby linking the iterator state
...
\begin{small}\begin{verbatim}
Fixpoint itp(t:type)(it:iter t):st_context :=
   match it with
   | TIPrim _ => nil
   | TITuple t1 t2 it1 it2 => (itp t1 it1) ++ (itp t2 it2)
   | TIUnionL t1 _ it1 => false :: (itp t1 it1)
   | TIUnionR _ t2 it1 => true :: (itp t2 it1)
   end.
\end{verbatim}\end{small}
