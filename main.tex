...
The standard subtyping algorithm for the combination of distributive union and
tuple types has, for some time, been exponential in both time and space. An
alternative algorithm, linear in space but still exponential in time, has been
tribal knowledge in the subtyping community~\cite{tateemail}. In this paper,
we describe and prove correct an implementation of this algorithm.
...
function return types, so a function's signature consists solely of the
tuple of its argument types. These tuples are covariant, as a function with
more specific arguments (a property decided using subtyping) is preferred to a
...
relative specificity must be decided using subtyping.

This paper documents the first steps towards proving the correctness of
Julia's subtyping algorithm. To do so, we first examine typical approaches
...
denotational model, and the resulting subtype algorithm can be proved both
...
tuples and unions that uses iterators instead of normalization.  This
...
Our next steps will be subtyping for primitive types, existential type
