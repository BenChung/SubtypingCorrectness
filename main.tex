...
Union types are playing an increasing role in mainstream languages.  In some cases, as Julia~\cite{BezansonEKS17} or TypeScript~\cite{}, they are exposed at the source level, in others, as Hack~\cite{}, they are only used internally when performing type inference; in all cases they play a key role in the expressiveness of the language.

Rules for subtyping union types have been known for a long time, and following Vouillon~\cite{Vouillon04} they may be written as:
\begin{mathpar}
...
\end{mathpar}
It has also been know for a long time that, in presence of covariant tuples, the above rules are not complete with respect a semantic subtyping model~\cite{}.  
 The standard example is to prove that covariant types are \emph{distributive} with respect to unions, that is that the following derivation holds:
\[
...
\]
then it is evident that the rule for tuple types does not apply, and derivations must immediately pick either the left or right component of the union type, and none allows to conclude.  The standard approach to solve this condrundum is to rewrite all types into their disjunctive normal form, that is, as unions of intersections of literals, \emph{before} building the derivation.  The rewriting step can be implemented easily and its correctness can be justified by a semantic-subtyping denotational model, as in~\cite{Frisch08}, and the resulting subtype algorithm can be proved correct and complete. However, the standard algorithm based on ahead-of-time normalisation has \emph{two major drawbacks}.  The first drawback is that the normalisation phase rewrites types into potentially \emph{exponentially bigger} types.  In practice, the Julia runtime must routinely solve queries involving \textbf{EXACT NUMBER FROM LOGS - find again the nice example where normalisation kills but Julia flies}; normalising these types before deciding subtyping is not an option.
 %: while subtyping union types is inherently exponential in time, it does not have to be \emph{exponential in space}.  

The other problem is that the ahead-of-time normalisation phase does not interact nicely with other type system features.  If we add invariant constructors for instance, then it is unsound to lift all union types at top level, because \(\arrayt{\union{t_1}{t_2}}\)  \emph{is not}  a subtype of \(\union{\arrayt{t_1}} {\arrayt{t_2}}\), XXX


More interestingly, if we extend the language with existential types and invariant constructors, then :

  Ref{Union{Tuple{Int}, Tuple{Bool}}} <: Ref{Tuple{T}} where T


While subtying union types in presence of covariant types is inherently exponential in time in the worst case, it is 

In this paper we describe how the algorithm used by the Julia language to
decide subtype solves subtyping union types and tuples, providing evidence that it does not suffer from the two drawbacks above.  We prove in
