...
we can generalize this pattern into two steps:

first step: we flip the deepest L-choice into an R-choice.
A lot like how we take the deepest 0 in a binary number and make it into a 1.

second step: the remainder of the list is truncated
this is because the old choices (which are all R-choices) following the
flipped choice don't make sense anymore

third step: we pad out the list with L-choices until we end up with a valid type again
we choose L-choices because the algorithm sweeps left-to-right. This is like filling the
binary number out with zeros; we could go the other way, which is essentially like decrementing 
in binary.

using this, we can define an initial state: the all-left configuration. It's like all zeros in binary.

can then use this to iterate through the type. in OCaml, we start with an all-left configuration made
by padding the empty list. then step using next and pad.

now we have a way to iterate through the list of base types we can now do subtyping with it
...
\caption{Subtyping decision procedure example}\label{fig:cfs}
\end{figure}

This algorithm works just as it did with the two lists of base types produced
by normalization, but iterating over the choices implied by the type AST
instead of the explicit base types. An example of its operation is shown in
figure~\ref{fig:cfs}, which depicts its operation over a subtype query. In
this example, both iterators start in the state ``L'', corresponding to the
base type $\tuple{A}$ on the left and $\tuple{A}$ on the right. These are trivially
subtypes, so this first case of the left-hand universal quantification is finished and
the left-hand iterator may now be stepped to find the next. 

Now that the left-hand iterator is taking the R choice, we restart the right-hand-side
iterator to see if there exists a choice for the right-hand union that makes a supertype
of the left-hand's $\tuple{B}$. The initial state for the right-hand-side is ``L'', 
inducing the type $\tuple{A}$, which is not a supertype of $\tuple{B}$. We can continue
in spite of this failure, as there remains one more step in the right-hand iterator, 
inducing $\tuple{A}$ from the choice R. Thus, we have found a choice on the right for
each choice on the left such that the left-hand base type is a subtype of the right-hand
base type.

We again present an implementation of this algorithm in OCaml. The core operation of
subtyping is subtyping between base types, which we implement by implicitly sub-setting
the full type AST given the lists of choices made on each side.
