...
need to get from normalization by calling {\tt next} with a type term and a
choice list. The initial choice list (all {\tt L}s) is obtained by function
{\tt initial} given a type term.


\begin{lstlisting}
type choice = L | R

let rec done = function
  | [] -> true
  | R::tl -> done tl
  | L::tl -> false
...
let step (l:choice list) =
  if done l then None else Some(rev (toggle (rev l)))
\end{lstlisting}
...
let rec initial(a:typ) = 
  match a with
   ...
   | Union(l,r) ->  L::(initial l) @ (initial r)
\end{lstlisting}
