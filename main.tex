...
Our algorithm for deciding subtyping for the type language
runs in exponential time but constant space. In general, it
works by iterating through every possible choice that could
be made between branches of unions within a type and incrementally
checking that the requisite guarantees hold at each type.
\hspace{1em}
...
Next, we need some way to take one list of choices inside a type and
step it forward to the next set of choices. We cannot simply treat the
list as a binary number, as by taking a different branch we need to make
a new set of choices for that branch. As a result, we break down the step
operation into two separate steps.

The first step for the iteration function is \verb|last_left_to_right|. This
function finds the final Left instance in a list of choices, converts it to
a Right instance appended to the rest of the list unmodified. This has us take
the deepest alternative branch. \verb|extend_list| then initializes the remainder
of the choice list with Left choices, giving a valid state. If no successor state is 
found - if there are no Left choices that remain - then the function will return None.

With a way to come up with an initial state and a way to step states, we can define
the \verb|subtype| function, which uses the helpers \verb|fa_ex_subtype| and \verb|ex_subtype| 
to check that for every instantiation of the left-hand-side, there exists an instantiation of
the right-hand-side such that \verb|base_subtype| holds.

\begin{verbatim}
type st_res =
  | Subtype of st_choice list * st_choice list * var_env
  | NotSubtype
...
let rec base_subtype (a:typ) (b:typ) (fa:st_choice list) (ex : st_choice list)
  ...
  | (Union(a1,a2),b,Left::fa,ex) -> base_subtype a1 b fa ex
  | (Union(a1,a2),b,Right::fa,ex) -> base_subtype a2 b fa ex
  | (a,Union(b1,b2),fa,Left::ex) -> base_subtype a b1 fa ex
  | (a,Union(b1,b2),fa,Right::ex) -> base_subtype a b2 fa ex
