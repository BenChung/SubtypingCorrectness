...
We use the iterator infrastructure to implement subtyping. 
The fundamental procedure is identical to the one with normalization: for each union free
type term in the normal form for the left hand type, we need to ensure that there exists
is a supertype union free type term on the right hand side. However, instead of precomputing
said union free type terms, we generate them while checking subtyping from the combination of a type and the 
choice string for that type.

The entry point is through \c{subtype}. It establishes the forall-exists relationship using
\c{allexists} starting from the left-hand initial state. 

\begin{lstlisting}
let subtype(a:typ)(b:typ) = allexists a b (initial a)
\end{lstlisting}

In order to ensure that the forall-exists relation between two types \c{a} and \c{b} holds starting from the
given iterate \c{f}, \c{allexists} needs to ensure that there is some choice list for \c{b} that 
supertype for the current iterate and that this will hold for the successive
states. It will check the former property using the \c{exists} function, which
when started with the initial state for the right-hand-side will find if there
is some supertype of the given left-hand-side type and choice string. Then, if this
passes, \c{allexists} will continue iterating. If no more states  remain in
the iterator, then \c{allexists} exits successfully.
...
Similarly, the \c{exists} function iterates through each option on the right-
hand, checking if it is a supertype of the given left-hand type and choice
string. It will first check if the current iterate is a supertype of the given
left-hand type using the \c{sub} function. If \c{sub} returns \c{IsSub}, then
the current right-hand choice string is a supertype of the given left-hand and
\c{exists} can exit successfully. Otherwise,  \c{exists} will recur on the
iterator until it either runs out of choice strings in the iterator (in which
case it fails) or it finds a supertype of the left.
...
Finally, the \c{sub} function decides if two types, defined by the base type term and 
