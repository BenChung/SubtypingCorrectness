...
Are these types subtypes? Under the semantic definition of subtyping, we need to ensure that for every
possible choice on the left hand side, there exists another choice on the right hand
side such that subtyping holds between them. If we use normalization, we compute
all of the choices ahead of time, illustrating this relation explicitly.

We can write out the options on both sides that are available to us after
normalizing. On the left hand side, the union of tuples gives us two clear
choices.
...
LHS & RHS \\
\hline
\tuple{p_1,p_2} & \tuple{p_2,p_3}\\
\tuple{p_2,p_3} & \tuple{p_2,p_2}\\
& \tuple{p_1,p_3}\\
& \tuple{p_1,p_2}\\
\end{array}
\]

The right hand side normalizes to the union of the following four tuples:
...
Subtyping can be defined with two helper functions, allexists, which
iterates through every left-hand side tuple and checks that for each
one, the second function, exists, finds at least one matching tuple
on the right hand side.

Each of these function takes as state a position in the sequences
of tuples.

The intuition is that instead of materializing those sequences of tuples
which would requires exponential space, we will iterate through that
space by walking over the abstract syntax trees of  type terms.

The state of the iterators is the encoding of position in the tree
that remembers all of the choice points, i.e. all of the unions that
were encountered.

Consider

 tuple union t3 t1 union t1 t2
...
Position 1 in the normalized sequence can be encoded  as LL,
position 2 is LR, 3 is RL and 4 is RR.

Each letter in the string represents one choice point.
