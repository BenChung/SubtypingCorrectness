...
let allexist (a:typ list)(b:typ list) = 
  foldl (fun acc a' => acc && exist a' b) true a

let exist (a:typ)(b:typ list) = 
...
type variables, and invariant type constructors. While not included in Julia,
arrow types would be another important inclusion, made difficult by the need
to go from a covariant to a contravariant environment and thus flip the
forall-exists contexts.
