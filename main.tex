...
\begin{abstract}
  ...
  ahead-of-time. However, normalization-based algorithms suffer from two problems: complex type signatures induce space explosion, and their interactions with other features of Julia’s type system render them incorrect. In this paper, we describe the algorithm that Julia
  uses to compute subtyping between tuples and unions---an algorithm that is immune to these issues of space explosion
  and expressiveness.  We prove this algorithm correct and complete against a semantic-subtyping denotational model in Coq.
\end{abstract}
...
others, such as Hack~\cite{hack}, they are only used as part of
type inference. As a result of this increasing use of union types both as
expressibility aids and for type inference, subtyping between union types is of increasing
practical import.

We describe a space-efficient technique for computing subtyping between types
that contain distributive unions, arising from the Julia programming
language. In our previous work on formalizing the Julia subtyping
algorithm~\cite{DBLP:NardelliBPCBV18}, we described the subtyping relation but
were unable to describe the subtyping algorithm or prove it correct. Indeed,
we found bugs and were left with unresolved issues.

Julia's subtyping algorithm~\cite{bezansonthesis} is an important part of its semantics. Julia is
...
determine which method to invoke at each call site. It searches for
the most specific (according to subtyping) applicable method that applies
for a given invocation. 
For example, the following snippet shows three declarations of
multiplication:
...
express complex relationships.  Julia's types include nominal single
...
In this paper, we will focus on the interaction of two features: covariant
tuples and union types. These two kinds of type are very important to Julia's
semantics, as both factor heavily into method dispatch. Julia does not record
function return types, so function types in Julia are only the tuple of their
argument types. These tuples are covariant, as a function with more specific
arguments (a property decided using subtyping) is preferred to a more generic
one. Union types are widely used as shorthand to avoid writing multiple
functions with the same body. As a consequence, Julia library developers write
many functions with union typed arguments, functions whose relative
specificity must be decided using subtyping.

This paper documents the first steps towards proving the correctness of
Julia's subtyping algorithm. To do so, we will first examine typical approaches
to decide subtyping in the presence of union types. Based on Vouillon~\cite{Vouillon04},
...
The standard solution is normalization. Normalization
...
semantic denotational model~\cite{Frisch08}, and the resulting
subtype algorithm can be proved both correct and complete.
...
other designs, as Vouillon~\cite{Vouillon04} or Dunfield~\cite{DBLP:journals/jfp/Dunfield14} do not handle distributivity. 
In modern approaches,  normalization remains popular. It is
relied upon by Frisch et al.\ work on semantic subtyping~\cite{Frisch08}, by Pearce's
...
expression inclusion. However, normalization 
...
other features of the type system. In particular, Julia supports type invariant
...
\noindent 
This judgment holds if we set the existential {\small$T=\union{t}{t'}$}, and therefore
our algorithm should conclude it holds. Since all types are in weak normal form, an algorithm based on the standard
...
(that {\small$\tuple{T}<:\union{\tuple{t}}{\tuple{t'}}$}).
...
Julia's subtyping algorithm avoids these problems. However, the complete Julia
subtype algorithm is implemented in close to two thousand lines of highly
optimized C code. In this paper, we describe and prove correct only one part
of that algorithm, the technique used to avoid space explosion while dealing
with union types and covariant tuples. This is done by defining an iteration
...
We have chosen a minimal type language with union, tuples, and primitive types to
...
this implementation technique known to a wider audience. Empirical evidence from
Julia's implementation suggests that this technique extends to invariant constructors
...
Formally, our core type language consists of binary unions, binary
tuples and primitive types ranged over by $p_1 \dots p_n$. We define 
subtyping for primitives as the identity, so $p_i <: p_i$.
...
\noindent
We can then convert these chains of normalized unions into sets of 
union-free types equivalent to each side,
...
\noindent Determining whether $\ell_1 <: \ell_2$ boils down to checking that for each element $t_1$ in $\ell_1$ there should be an element $t_2$ in $\ell_2$ such
that $t_1 <: t_2$. Checking this final relation is straightforward, as neither
$t_1$ nor $t_2$ may contain unions. Intuitively, this mirrors the above defined
...
choice points, each represented as a ? symbol:
\medskip
...
and $c''$ is a possibly empty sequence of {\R}s.  The next string is $c'\, \R
\, c_{pad}$, that is to say it retains the prefix $c'$, toggles the \L to an \R,
and is padded by a sequence of \Ls.  If there is no \L in $c$, iteration terminates.
...
returns a pair, whose first element is the padded string and second is the
string left over from the current type. \c{pad} traverses the term, exploring 
both sides of each tuple encountered and the choice indicated by the given string
when encountering a union. Each union thereby consumes a character from the input string.
Unions explored after the exhaustion of the original choice string are treated as if there
was a \L remaining in the choice string. The first component of the returned value is the choice string given as
argument extended with a number of {\L}s corresponding to the number of unions
encountered after the original choice string ran out.
...
the general framework for which we will prove the iterator-based algorithms correct.

The order in which choice lists iterate through a type term is determined by
both the choice list and the type term being iterated over. Rather than
...
successor state or is a final state. Recalling the graphical notation of
...
child. When given a \c{ILeft} or an \c{IRight} it tries to step its only
...
type term. A \c{IRight} whose child has no successor state will also have
no successor.
...
subtyping between the resultant union-free types. These implementations are
equivalent, as they both apply the given choice lists at the same places while
computing subtyping; however, the proof version separates choice list
application while the implementation intertwines it with the actual subtyping
decision.
