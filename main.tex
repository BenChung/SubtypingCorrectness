...
the iterator at every level encountered even if no union is present. Notably,
this is not correct Coq code: the function is not simply recursive and the Coq
termination checker cannot guarantee termination. To avoid this, we use 
\c{Program Fixpoint} using the sum of the sizes of \c{a} and \c{b} as the
metric.
