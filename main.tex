...
the desired subtype function is \verb|forall t t':type, {NormalSubtype t t'} +
{~NormalSubtype t t'}|.

Subtyping with normalization is simple to prove correct (see
\verb|normalize_subtype| in the Coq source for full detail); the normalization
...
proving that by building iterators over choices made at explicitly specified
nodes in the type tree, we can visit the same list of union-free types that
normalization produces. We then prove the same property of list-based
iterators by showing equivalence to tree-based iterators, and conclude by
proving that the three methods all can be used to decide subtyping in a sound
and complete manner.
