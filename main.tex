...
Using \verb|iter_rect|, we can implement and prove correct equivalent
functions to \verb|exists|, and \verb|allexists|. Therefore, we can prove a
version of \verb|subtype| based on iteration over choice trees. The \c{exists}
and \c{allexists} functions work in the same way that they did for normalized
lists of  terms, but instead of computing with union free type terms directly
they iterate through \c{iter}s for \c a and \c b. These iterators, initalized
by \c{init} to follow the structure of the type while taking the left choice
at each choice point, are used in the \c{sub}  function as choice strings
were.

We consider the denotation of a tree iterator state $|\c{i}|$ to be the
set of reachable union-free types from the current state. We also write $\c a(\c {ia})$ 
to indicate the union-free type produced from the type \c a by following the choices in
the tree iterator \c{ia}.

\begin{lemma}[Tree iterator denotational equivalence]
$|\c{initial a}| = \denotes{\c a}$    
\end{lemma}

\begin{theorem}
\c{exist a b ia ib} holds iff $\exists \c t \in |\c{ib}|, \c t = \c a(\c{ia})$
\end{theorem}

\begin{theorem}
\c{allexist a b ia} holds iff $\forall \c{a'} \in |\c{ia}|, \exists \c{b'} \in \denotes{\c b}, \c{a'} = \c{b'}$
\end{theorem}
...
let rec sub = function
    ...
    | (Union(a,_), b, ILeft _ _ ia, ib) -> sub a b ia ib
    | (Union(_,a), b, IRight _ _ ia, ib) -> sub a b ia ib
    | (a, Union(b,_), ia, ILeft _ _ ib) -> sub a b ia ib
    | (a, Union(_,b), ia, Right _ _ ib) -> sub a b ia ib
...
let subtype(a b:typ) = allexists a b (initial a)
\end{lstlisting}

\c{exist} is equivalent to the choice string based \c{exist}, and determines
if there exists some denotationally-contained type in \verb|b| that is a
supertype of the given \verb|a|. Internally, it is implemented by using
\verb|iter_rect| to perform induction over the list of remaining states in
the iterator for \verb|b|. If no states remain, then trivially the failing
case holds.  Otherwise, it will determine if the current iterator state is a
supertype of the given \verb|a|. If it is, then it can terminate early with
a successful (left) outcome. Otherwise, it continues with the IH.
