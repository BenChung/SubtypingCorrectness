...
let last_left_to_right (l:choice list) =
...
\begin{lstlisting}
let rec pad(a:typ)(ls:choice list) = match (a,ls) with
 | (Atom i,_) -> ([],ls)
...
\begin{lstlisting}
let rec next(a:typ)(ls:choice list) =
  option_map fst (option_map (pad a) (last_left_to_right ls))
\end{lstlisting}
...
\begin{lstlisting}
type res =
 | Subtype of choice list * choice list
 | NotSubtype
\end{lstlisting}

The type \verb|res| represents the results of a single base subtype query. The query
can either succeed, in which case it provides the unused portion of the input
stacks, or fail. 

\begin{lstlisting}
let rec base_subtype(a:typ)(b:typ)(fa:choice list)(ex:choice list)
...
\begin{lstlisting}
let rec exists(a:typ)(b:typ)(fa:choice list)(cex:choice list) =
  match base_subtype a b fa cex with 
  | Subtype(_,_) -> true 
  | NotSubtype -> 
     (match next b cex with
      | Some ns -> exists a b fa ns 
      | None -> false) 
\end{lstlisting}

If the current choice---given by \verb|cex|---is a supertype of the given
\verb|a| according to \verb|base_subtype|, then \verb|exists| has found
...
instantiations.
 ...
 \begin{lstlisting}
let rec allexists(a:typ)(b:typ)(cfa:choice list) =
  match exists a b cfa (initial b) with 
  | true -> (match next a cfa with
