...
\begin{abstract}
  ...
  subtyping between method signatures to pick the invoked method. Julia's
  ...
  ahead-of-time. However, normalization is not practical due to
  complex type signature space-explosion and interactions with other features of Julia's
  type system.  Our contribution is a description of the algorithm
  implemented in the Julia run-time system, immune to these issues of space-explosion and expressiveness.  We
...
type inference. As a result, subtyping between union types is of increasing
practical import.

We describe a space-efficient technique for computing subtyping between types
in the presence of distributive unions, arising from the Julia programming
...
to decide which of the methods will be invoked for any specific invocation; the call
...
libraries, but subtyping between them poses substantial challenges, challenges
that arise  due to Julia's semantic subtyping inpsired
design~\cite{BezansonEKS17}. Subtyping in Julia follows the semantic intuition
---that subtyping between two types should be equivalent to  their denoted
sets of values being subsets of one another---a property that is difficult to
establish algorithmically for a type language as rich as Julia's.
...
covariant tuples and union types. Tuples are used to represent function
